...
export class ContactosProvider {
  ...
  public insert(id, nombre, numero, sms) {
    return this.db.transaction(
      (tsql) => {
        tsql.executeSql(
          'INSERT INTO junt_contactos (cont_id,cont_nombre,cont_numero,cont_sms) ' +
            ' VALUES (?1,?2,?3,?4)',
          [id, nombre, numero, sms]
        );
      },
      (Terror) => {},
      () => {
  ...
  public update(id, nombre, numero, sms) {
    return this.db.transaction(
      (tsql) => {
        tsql.executeSql(
          'UPDATE junt_contactos SET cont_nombre=?,cont_numero=?,cont_sms=? ' +
            ' WHERE cont_id=?',
          [nombre, numero, sms, id]
        );
      },
      (Terror) => {},
      () => {
  ...
  public findAll() {
    return new Promise((resolve, reject) => {
      ...
      this.db.transaction(
        (tsql) => {
          tsql.executeSql(
            'SELECT cont_id as id,cont_nombre as displayName,cont_numero as phoneNumber,cont_sms as sms ' +
              ' FROM junt_contactos ' +
              ' ORDER BY cont_nombre asc',
            [],
            (tsql, resp) => {
  ...
  public getById(id) {
    return this.db.transaction((tsql) => {
      tsql.executeSql(
        'SELECT cont_id,cont_nombre,cont_numero,cont_sms ' +
          ' FROM junt_contactos  WHERE cont_id = ?',
        [id],
        (tsql, resp) => {
          //console.log(resp);
        }
